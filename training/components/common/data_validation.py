...
class DataValidation:
    ...
    def validate_data(self):
        ...
        validate the column names and dtype of ingested data
        ...
        try:
            ...
            for i,j in zip(col_dtypes.index,schema.keys()):
                if i==j:
                    ...
                    with open(self.config.STATUS_FILE, "w") as f:
                            f.write(f"Data Validation status {status}")
            info_logger.info("Data Validation Component complete")
...
if __name__ == "__main__":
    # Initialize the ConfigurationManager and retrieve the data validation config
    config = ConfigurationManager()
    data_validation_config = config.get_data_validation_config()

    # Create data validation object and call the save_data method
